                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input className="input" placeholder="e.g., cinematic, slow motion, panning, aerial"
                             value={(value.videoHints?.style||[]).join(', ')}
                             onChange={(e)=>set('videoHints', { ...(value.videoHints||{}), style: sanitizeWords(e.target.value.split(',')) })} />
                      <p className="text-xs text-ink/60 mt-1">Describe motion and camera work: panning, tracking, aerial, drone, slow-motion, timelapse, etc.</p>
                    </div>
                    <div>
                      <label className="label">Tech tags (comma-separated)</label>
                      <input className="input" placeholder="e.g., 4k, 60fps, hd"
                             value={(value.videoHints?.tech||[]).join(', ')}
                             onChange={(e)=>set('videoHints', { ...(value.videoHints||{}), tech: sanitizeWords(e.target.value.split(',')) })} />
                      <p className="text-xs text-ink/60 mt-1">Technical specs: 4K, 60fps, HD, etc. Only include if accurate.</p>
                    </div>
                  </div>
                </div>
              )}
